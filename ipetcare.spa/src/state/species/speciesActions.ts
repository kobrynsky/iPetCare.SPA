...
interface GetAllSpecies {
  ...
  export const createRace = (speciesUnit: Species): ThunkResult<void> => async dispatch => {
    handleCreateSpecies(dispatch)
    try {
      const response: AxiosResponse<Species> = await species.create(speciesUnit)
      handleCreateSpeciesSuccess(dispatch, response.data)
    } catch (e) {
  ...
  export const updateSpecies = (
    ...
    try {
      const response: AxiosResponse<Species> = await species.update(updatedSpecies)
      handleUpdateSpeciesSuccess(dispatch, response.data)
    } catch (e) {
